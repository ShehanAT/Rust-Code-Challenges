mod vigenere {
    ...
    pub fn encrypt(plaintext: &str, key: &str) -> String {
        let mut key_bytes = key.bytes().map(|x| x - A).cycle(); // The purpose for x - A seems to be for encryption purposes
    ...
    pub fn decrypt(ciphertext: &str, key: &str) -> String {
        let mut key_bytes = key.bytes().map(|x| x - b'A').cycle(); // The purpose for x - A seems to be for decryption purposes
